...
//          slot1_bank          <= #1 2'b00;
        ...
        // request from read cache
          ...
          ba                  <= #1 cpuAddr_mangled[22:21];
          slot1_bank          <= #1 cpuAddr_mangled[22:21];
          cas_dqm             <= #1 {cpuU,cpuL};
          sd_cs               <= #1 4'b1110; // ACTIVE
          sd_ras              <= #1 1'b0;
      ...
      ph4 : begin
        sdaddr                <= #1 {5'b00100, casaddr[8:1]}; // AUTO PRECHARGE
        ba                    <= #1 casaddr[22:21];
        sd_cs                 <= #1 cas_sd_cs;
        if(!cas_sd_we) begin
          dqm                 <= #1 cas_dqm;
      ...
      ph9 : begin
        ...
        if(!refresh_pending && slot1_type != REFRESH) begin
          if(writebuffer_req && |writebufferAddr[22:21] && (slot1_type == IDLE || slot1_bank != writebufferAddr[22:21])) begin // reserve bank 0 for slot 1
            // We only yield to the OSD CPU if it's both cycle-starved and ready to go.
            slot2_type        <= #1 CPU_WRITECACHE;
            sdaddr            <= #1 {1'b0, writebufferAddr[20:9]};
            ba                <= #1 writebufferAddr[22:21];
            slot2_bank        <= #1 writebufferAddr[22:21];
            cas_dqm           <= #1 writebuffer_dqm;
            sd_cs             <= #1 4'b1110; // ACTIVE
            sd_ras            <= #1 1'b0;
            casaddr           <= #1 {writebufferAddr[24:1], 1'b0};
          ...
          // request from read cache
          else if(cache_req && |cpuAddr_mangled[22:21] && (slot1_type == IDLE || slot1_bank != cpuAddr_mangled[22:21])) begin // reserve bank 0 for slot 1
            slot2_type        <= #1 CPU_READCACHE;
            sdaddr            <= #1 {1'b0, cpuAddr_mangled[20:9]};
            ba                <= #1 cpuAddr_mangled[22:21];
            slot2_bank        <= #1 cpuAddr_mangled[22:21];
            cas_dqm           <= #1 {cpuU, cpuL};
            sd_cs             <= #1 4'b1110; // ACTIVE
            sd_ras            <= #1 1'b0;
            casaddr           <= #1 {cpuAddr_mangled[24:1], 1'b0};
